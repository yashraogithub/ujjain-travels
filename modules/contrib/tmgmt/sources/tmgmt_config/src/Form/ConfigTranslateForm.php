  ...
  public function buildForm(array $form, FormStateInterface $form_state, ?array $build = NULL, $plugin_id = NULL) {
    ...
    foreach ($languages as $langcode => $language) {
      ...
      elseif (isset($items[$langcode])) {
        ...
        $additional = Link::fromTextAndUrl($item->getStateIcon() ?: $states[$item->getState()], $item->toUrl()->setOption('query', $this->getDestinationArray()))->toString();
      ...
      if (isset($option['data'])) {
        ...
        $form['languages']['#options'][$langcode] = array(
          \Drupal::service('renderer')->render($option['language']),
          $additional,
          \Drupal::service('renderer')->render($option['operations']),
        );
      }
    }
    return $form;
  ...
  function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach (array_keys(array_filter($values['languages'])) as $langcode) {
      ...
      try {
        ...
        Error::logException(\Drupal::logger('tmgmt'), $e);
        $languages = \Drupal::languageManager()->getLanguages();
